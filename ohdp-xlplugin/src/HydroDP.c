...
void* dll_allocblock( unsigned int uBytes )
{
	...
	BYTE* p = malloc( uLinkSize + uBytes );
	if( !p ) { return 0; }
...
//		_set_SSE2_enable(1);
		...
		if( !gszInfo )
		{
			gszInfo = calloc( strlen( ADDIN_NAME " " HDP_VNUM "/" DP_VNUM " " BUILDTYPE ) + 1, 1 );
    ...
    for( pszArg = rgFuncs; *pszArg != 0; pszArg += 10 )
	{
		...
		for( i = 0; i < 10; i++ )
		{
			...
			if( i == 4 )
			{
				...
				xloper_int( &(xoper[i]), atoi( (char*)(pszArg[i]) ) );
			}
			else
...
__declspec(dllexport) LPXLOPER WINAPI xlAddInManagerInfo(LPXLOPER pxAction)
{
#if 0
	XLOPER xIntVal;
...
__declspec(dllexport) void WINAPI xlAutoFree(LPXLOPER pxFree)
{
	xloper_free( pxFree );
	return;
}
