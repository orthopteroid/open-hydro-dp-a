...
/****************************************************/
	...
	#define NAN (log(-1.0))
		...
		#define EXTERNC extern "C"
			...
			#define PRINT_STDERR( _sz ) do { OutputDebugStringA( _sz ); } while(false);
			#define PRINT_STDOUT( _sz ) do { OutputDebugStringA( _sz ); } while(false);
		#endif
	#else
		#define EXTERNC
		#define FLOAT float

		#define PRINT_STDERR( _sz ) do { fprintf( stderr, _sz ); } while(false);
		#define PRINT_STDOUT( _sz ) do { fprintf( stdout, _sz ); } while(false);
	#endif
#else
	#include <unistd.h>
...
#ifdef _DEBUG
	#define VERIFY( x ) do { if( !ex_didFail() ) { assert( x ); if( !(x) ) { ex_set(); } } } while(0)
#else
...
#define ENABLE_NUMERICAL_CLEANUP
...
#define ENABLE_MWOBJECTIVE
#define ENABLE_ADAPTIVEFAILURE
#define ENABLE_HK_SCALING // changes solution

//#define DEBUG_BACKWARDPASS
//#define DEBUG_FORWARDPASS
//#define DEBUG_MEMORY
//#define DEBUG_OBJECTIVE_FUNCTION
...
CHAR* float_format( FLOAT aFloat )
{
    ...
    } else if( fAbsFloat >= 100000 ) {	return "%.0f";
    } else if( fAbsFloat >= 10000 ) {	return "%.1f";
	} else if( fAbsFloat >= 1000 ) {	return "%.2f";
	} else if( fAbsFloat >= 100 ) {		return "%.3f";
	} else if( fAbsFloat >= 10 ) {		return "%.4f";
	} else if( fAbsFloat >= 0.001 ) {	return "%.5f";
...
FLOAT float_round( FLOAT fVal, UINT32 uSigfigs )
{
	...
	FLOAT fIntPart = 0;
	modf( fabs( fSigNif ) + 0.5, &fIntPart ); /* round to number with larger magnitude, -ve or +ve */
...
{
	UINT32 i;
	for( i=0; i<num; i++ ) { pDest[i] = pSrcN[i] / pSrcD[i]; }
}
...
void arr_float_sum_stages( FLOAT* pDest, FLOAT* pSrc, UINT32 uStates, UINT32 uStages )
{
	FLOAT fTol = 1E-3;
	UINT32 i, j;
	for( i=0; i<uStates; i++ )
	{
		...
		for( j=0; j<uStages; j++ )
		{
			...
			if( !isnanf( v ) && !isinff( v ) && (fabs( v ) > fTol) ) { pDest[i] += v; }
		}
	}
}

void arr_float_average_stages( FLOAT* pDest, FLOAT* pSrc, UINT32 uStates, UINT32 uStages )
{
	FLOAT fTol = 1E-3;
	UINT32 i, j, k;
	for( i=0; i<uStates; i++ )
	{
		...
		{
			FLOAT v = pSrc[ j * uStates + i ];
			if( !isnanf( v ) && !isinff( v ) && (fabs( v ) > fTol) ) { pDest[i] += v; k++; }
		}
		if( k > 0 ) { pDest[i] /= k; }
	}
}
...
void arr_float_clear_conditional( FLOAT* pDest, UINT32* pTest, UINT32 num )
{
	UINT32 i;
...
DP_INLINE void arr_float_copy_indirect( FLOAT* pDest, FLOAT* pSrc, UINT32* pMap, UINT32 num )
{
	UINT32 i;
	for( i=0; i<num; i++ ) {
...
DP_INLINE void arr_float_max( FLOAT* pfMaxValue, UINT32* puIndex, FLOAT* pFloat, UINT32 count, UINT32 uStride )
{
...
DP_INLINE void arr_float_tol( FLOAT* pfTolValue, UINT32* puIndex, FLOAT* pFloat, UINT32 uOptimum, FLOAT fTol, UINT32 count, UINT32 uStride )
{
	FLOAT fOptimum = pFloat[ uOptimum ];
	FLOAT fTolValue = fOptimum;
	UINT32 uIndex = uOptimum;
	UINT32 i;
	for( i = uOptimum-1; i != MAX_UINT; i-- )
	{
		FLOAT v = pFloat[ i * uStride ];
		if( v >= fOptimum * fTol ) { uIndex = i; fTolValue = v; } else { break; }
	}
	*pfTolValue = fTolValue;
	*puIndex = uIndex;
}

DP_INLINE void arr_float_last_nonzero( FLOAT* pfLastNonZeroValue, UINT32* puIndex, FLOAT* pFloat, UINT32 num, UINT32 uStride )
{
	FLOAT fTol = 1E-6;
	FLOAT fLastNonZeroValue = 0;
	UINT32 uIndex = MAX_UINT;
	UINT32 i;
	for( i=0; i<num; i++ )
	{
		FLOAT v = pFloat[ i * uStride ];
		if( v > fTol ) { uIndex = i; fLastNonZeroValue = v; }
	}
	*pfLastNonZeroValue = fLastNonZeroValue;
	*puIndex = uIndex;
}

DP_INLINE void arr_float_max_unmarked_prioritized( FLOAT* pf, UINT32* pu, FLOAT* pValues, UINT32* pMarklist, UINT32* pPriorityOrder, UINT32 uStages, UINT32 uStride )
{
	FLOAT fNextBestValue = 0;
	UINT32 uNextBestStage = MAX_UINT; // init to 'error state'
#ifdef ENABLE_FORWARDPASS_PRIORITY_ORDER_SCAN
	for( UINT32 s = uStages - 1; s != MAX_UINT; s--)
#else
	for( UINT32 s = 0; s < uStages; s++)
#endif
	{
		UINT32 uPrioritizedStage = pPriorityOrder[ s ];
		FLOAT v = pValues[ uPrioritizedStage * uStride ]; // TODO: cache ick
		if( v > fNextBestValue && (pMarklist[ uPrioritizedStage ] != MAX_UINT) )
		{
			fNextBestValue = v;
			uNextBestStage = uPrioritizedStage;
		}
	}
	*pf = fNextBestValue;
	*pu = uNextBestStage;
...
EXTERNC UINT32 turbine_register_n( UINT32 uType, FLOAT h, FLOAT q, FLOAT p )
{
	...
	gpTurbs[ guTurbs ].uGenCurveNum = MAX_UINT;
	guTurbs++;
	return guTurbs-1;
}
...
EXTERNC void turbine_geneff( UINT32 uTurbine, FLOAT fGenEfficiency )
{
	...
	gpTurbs[ uTurbine ].uGenCurveNum = MAX_UINT;
}
...
void turbine_list()
{
	...
	for( uTurb = 0; uTurb < guTurbs; uTurb++ )
	{
		...
		fprintf( stdout, " gencurve %s ", (gpTurbs[ uTurb ].uGenCurveNum == MAX_UINT) ? "nil" : gpCurves[ gpTurbs[ uTurb ].uGenCurveNum ].szName );
		fprintf( stdout, "\n" );
	}
}
...
DP_INLINE FLOAT turbine_power( UINT32 uTurb, FLOAT hCurrent, FLOAT qCurrent )
{
	...
	FLOAT fTol = 1E-3;
	/**/
	FLOAT fGenEfficiency;
	if( gpTurbs[ uTurb ].uGenCurveNum != MAX_UINT )
	{ fGenEfficiency = curve_eff_from_qfact( gpTurbs[ uTurb ].uGenCurveNum, qCurrent / qMax ); }
	else
	{ fGenEfficiency = gpTurbs[ uTurb ].fGenEfficiency; }
	/**/
	...
	/**/
	FLOAT pCurrent = fEffAdjFact * fEff * ( qCurrent * hCurrent * gfConvFactor ) * fGeneratorAndYardLoss;
	pCurrent = MAX( 0, pCurrent ); /* sanity check */
...
DP_INLINE FLOAT turbine_discharge( UINT32 uTurb, FLOAT hCurrent, FLOAT pCurrent )
{
	...
	FLOAT fGenEfficiency;
	if( gpTurbs[ uTurb ].uGenCurveNum != MAX_UINT )
	{ fGenEfficiency = curve_eff_from_qfact( gpTurbs[ uTurb ].uGenCurveNum, pCurrent / pMax ); } /* note: approx */
	else
	{ fGenEfficiency = gpTurbs[ uTurb ].fGenEfficiency; }
	/**/
#ifdef ENABLE_NEW_DYNLOSS
	...
	int bEffSmallerThanTol = ( fEff < fTol );
	/**/
	FLOAT qCurrent = pTurbine / ( fEffAdjFact * fEff * ( hCurrent * gfConvFactor ) );
	qCurrent = MAX( 0, qCurrent ); /* sanity check */
	int bDischExceedsMax = ( qCurrent > qMax );
#else /* ENABLE_NEW_CALCS */
	FLOAT pMax = gpTurbs[ uTurb ].fMaxPower;
	FLOAT qMax = gpTurbs[ uTurb ].fMaxFlow;
...
UINT32* gpuScenarioDecisionStates = 0;

/* these are the accumulated value of particular state decisions made in the current stage */
UINT32* gpuStateOfMax = 0; /* [ states ] but also known as [ scenario] */
...
UINT32* gpuGlobalDecisionAllocationStates = 0; /* [ stages, states ] */
FLOAT* gpfGlobalDecisionAllocations = 0; /* [ stages, states ] */

UINT32* gpuSolution = 0; /* [ stages, states ] */
...
UINT32* gpuLocalNearOptimumStates = 0; /* [ stages ] */
UINT32* gpuLocalMaximumStates = 0; /* [ stages ] */
UINT32* gpuLocalOptimumStates = 0; /* [ stages ] */
UINT32* gpuStageChecklist = 0; /* [ stages ] */
UINT32* gpuStageChecklistCopy = 0; /* [ stages ] */

UINT32* gpuLocalDecisions = 0; /* [ states ] */
FLOAT* gpfStageMetric = 0; /* [ states ] */
UINT32* gpuStageMap = 0; /* [ stages ] */
...
FLOAT gfTotalPower[ 2 ], gfTotalFlow[ 2 ]; /* max plant power output */

/* [0] == faceplate capacity, head-adjusted and adjusted for overcapacity */
/* [1] == faceplate capacities, adjusted for overcapacity */
FLOAT gfMaxPower[ 2 ], gfMaxFlow[ 2 ]; /* max plant discharge */
FLOAT gfMinMaxPower[ 2 ], gfMinMaxFlow[ 2 ]; /* largest output of the smallest unit */
...
EXTERNC void dp_assign_weights()
{
	...
	for( uTurb=0; uTurb<guTurbs; uTurb++ )
	{
		...
		for( u = 0; u < 2; u++ )
		{
			...
			fPCapFact = gpTurbs[ uTurb ].fGenCapacity;
			fQCapFact = gpTurbs[ uTurb ].fGenCapacity;
			...
			gfMinMaxPower[u] = MIN( p[u] * fPCapFact, gfMinMaxPower[u] );
			gfMaxFlow[u] = MAX( q[u] * fQCapFact, gfMaxFlow[u] );
			gfMinMaxFlow[u] = MIN( q[u] * fQCapFact, gfMinMaxFlow[u] );
...
EXTERNC void dp_resize()
{
...

	{
		...
		/* initialize */
		UINT32 uIter = 0;
		FLOAT fBestSquare = 4e4;
		UINT32 uBestStepCount = 0;

		UINT32 uStepCount = guProblemStepCount;
		FLOAT fCommonStepSize = fMinMaxDecision / ( uStepCount - 1 );

		/* determine the value that factors at least uStepCount times */
		VERIFY( fMinMaxDecision > uStepCount );
		while( uStepCount < 200 && fCommonStepSize > 10.0 ) /* tuning params */
		{
			fCommonStepSize = fMaxDecision / ( uStepCount - 1 );
			{
		...
		guProblemStepCount = ( uBestStepCount == 0 ) ? guProblemStepCount : uBestStepCount; /* failsafe */
		fCommonStepSize = fMaxDecision / ( guProblemStepCount - 1 );
...
EXTERNC void dp_malloc()
{
	...
	DP_MALLOC_CACHE( gpuScenarioDecisionStates, UINT32, guStates * guStates, __FILE__, __LINE__ ); /* [ states, states ] but actually triangular */
	...
	DP_MALLOC_CACHE( gpuGlobalDecisionAllocationStates, UINT32,guStages * guStates, __FILE__, __LINE__ ); /* [ stages, states ] */
	DP_MALLOC_CACHE( gpfGlobalDecisionAllocations, FLOAT, guStages * guStates, __FILE__, __LINE__ ); /* [ stages, states ] */
	DP_MALLOC_CACHE( gpuSolution, UINT32, guStages * guStates, __FILE__, __LINE__ ); /* [ stages, states ] */
	...
	DP_MALLOC_CACHE( gpuStateOfMax, UINT32,guStates, __FILE__, __LINE__ ); /* [ states ] */
	...
	DP_MALLOC_CACHE( gpuLocalNearOptimumStates, UINT32, guStages, __FILE__, __LINE__ ); /* [ stages ] */
	DP_MALLOC_CACHE( gpuLocalMaximumStates, UINT32, guStages, __FILE__, __LINE__ ); /* [ stages ] */
	DP_MALLOC_CACHE( gpuLocalOptimumStates, UINT32, guStages, __FILE__, __LINE__ ); /* [ stages ] */
	DP_MALLOC_CACHE( gpuStageChecklist, UINT32, guStages, __FILE__, __LINE__ ); /* [ stages ] */
	DP_MALLOC_CACHE( gpuStageChecklistCopy, UINT32, guStages, __FILE__, __LINE__ ); /* [ stages ] */

	DP_MALLOC_CACHE( gpuLocalDecisions, UINT32, guStates, __FILE__, __LINE__ ); /* [ states ] */
	DP_MALLOC_CACHE( gpfStageMetric, FLOAT, guStates, __FILE__, __LINE__ ); /* [ states ] */
	DP_MALLOC_CACHE( gpuStageMap, UINT32, guStages, __FILE__, __LINE__ ); /* [ stages ] */
...
EXTERNC void dp_cleanup()
{
	...
	DP_FREE_CACHE( gpuStateOfMax, UINT32 );
	DP_FREE_CACHE( gpfValueOfMax, FLOAT );
	DP_FREE_CACHE( gpfSolutionAllocations, FLOAT );
	DP_FREE_CACHE( gpfFlowAllocations, FLOAT );

	DP_FREE_CACHE( gpuLocalNearOptimumStates, UINT32 );
	DP_FREE_CACHE( gpuLocalMaximumStates, UINT32 );
	DP_FREE_CACHE( gpuLocalOptimumStates, UINT32 );
	DP_FREE_CACHE( gpuStageChecklist, UINT32 );
	DP_FREE_CACHE( gpuStageChecklistCopy, UINT32 );

	DP_FREE_CACHE( gpuLocalDecisions, UINT32 );
	DP_FREE_CACHE( gpfStageMetric, FLOAT );
	DP_FREE_CACHE( gpuStageMap, UINT32 );
}
...
int dp_fnStageMetricComparison(const void* pA, const void* pB)
{
#if 1 // removes typecast
	FLOAT a = gpfStageMetric[ *(UINT32*)pA ];
	FLOAT b = gpfStageMetric[ *(UINT32*)pB ];
#else
	UINT32 a = gpfStageMetric[ *(UINT32*)pA ];
	UINT32 b = gpfStageMetric[ *(UINT32*)pB ];
#endif
...
EXTERNC void dp()
{
...

	{
		...
		for( uStage = 0; uStage < guStages; uStage++ ) /* par */
		{
			if( gpTurbs[ uStage ].fWeight < fTol ) {
				...
				arr_float_step(
					&(pIndepVariable[ uStage * guProblemStepCount ]), 0 /* TODO: min */, gfSolutionDelta,
...
#ifdef ENABLE_NUMERICAL_CLEANUP
	for( uStage = 0; uStage < guStages; uStage++ ) /* par */
	{
...
#ifdef DEBUG_OBJECTIVE_FUNCTION
	sprintf( gcPrintBuff, "\n%s%c\n", "gpfPowerAllocations", gcOutputDelimiter );
	PRINT_STDOUT( gcPrintBuff );
	for( uStage = 0; uStage < guStages; uStage++ )
	{
		sprintf( gcPrintBuff, "%s%c", gpTurbs[ uStage ].szName, gcOutputDelimiter );
		PRINT_STDOUT( gcPrintBuff );
		arr_float_print( stdout, &(gpfPowerAllocations[ uStage * guProblemStepCount ]), guProblemStepCount, gcOutputDelimiter );
	}

	sprintf( gcPrintBuff, "\n%s%c\n", "gpfFlowAllocations", gcOutputDelimiter );
	PRINT_STDOUT( gcPrintBuff );
	for( uStage = 0; uStage < guStages; uStage++ )
	{
		sprintf( gcPrintBuff, "%s%c", gpTurbs[ uStage ].szName, gcOutputDelimiter );
	...
	sprintf( gcPrintBuff, "\n%s%c", "gpfHKTableValues", gcOutputDelimiter );
	PRINT_STDOUT( gcPrintBuff );
	...
	for( uStage = 0; uStage < guStages; uStage++ ) /* par */
	{
		...
		arr_float_max(
			&(fTemp),
			&(gpuLocalOptimumStates[ uStage ]),
			&(gpfHKTableValues[ uStage * guProblemStepCount ]),
			guProblemStepCount,
			1 );
		VERIFY( gpuLocalOptimumStates[ uStage ] != MAX_UINT );
		if( ex_didFail() )
		{
	...
	{
		FLOAT fTemp;
		arr_float_last_nonzero(
			&(fTemp),
			&(gpuLocalMaximumStates[ uStage ]),
			&(gpfHKTableValues[ uStage * guProblemStepCount ]),
			guProblemStepCount,
			1 );
		VERIFY( gpuLocalMaximumStates[ uStage ] != MAX_UINT );
		if( ex_didFail() )
		{
			sprintf( gcPrintBuff, "unrealistic problem: flat efficiency curve.\n" );
			PRINT_STDOUT( gcPrintBuff );
	...
	{
		FLOAT fTemp;
		UINT32 uOptimum = gpuLocalOptimumStates[ uStage ];
		arr_float_tol(
			&(fTemp),
			&(gpuLocalNearOptimumStates[ uStage ]),
			&(gpfHKTableValues[ uStage * guProblemStepCount ]),
			gpuLocalOptimumStates[ uStage ],
	...
	{
		UINT32 u = 0;
		arr_uint32_sum( &u, gpuLocalMaximumStates, guStages, 1 );
		VERIFY( u != 0 );
		if( ex_didFail() )
		{
	...
	for( uStage = 0; uStage < guStages; uStage++ ) /* par */
	{
		...
		if( uPoints < 2 ) { continue; } // skip small curves
		//
		...
		int bOK1 = (fEff8 > fEff9) & (gpuLocalOptimumStates[ uStage ] < gpuLocalMaximumStates[ uStage ]);
		// if last point is same or best efficiency then optimum point should be last point
		int bOK2 = (fEff8 <= fEff9) & (gpuLocalOptimumStates[ uStage ] == gpuLocalMaximumStates[ uStage ]);
		if( !( bOK1 ^ bOK2 ) )
		{
			sprintf( gcPrintBuff, "condition on unit %s: maximum operating point at H/K of %f and optimum operating point at H/K of %f.\n",
				...
				gpfHKTableValues[ uStage * guProblemStepCount + gpuLocalMaximumStates[ uStage ] ],
				gpfHKTableValues[ uStage * guProblemStepCount + gpuLocalOptimumStates[ uStage ] ] );
			PRINT_STDOUT( gcPrintBuff );
		}
	}
	...
	for( uStage = 0; uStage < guStages; uStage++ )
	{
 	...
 	qsort( gpuStageMap, guStages, sizeof(UINT32), dp_fnStageMetricComparison );
#endif
	...
	{
		uStage = guStages - 1;
		uPrioritizedStage = gpuStageMap[ uStage ];

		FLOAT* pfGlobalDecisionValues = &(gpfGlobalDecisionValues[ uPrioritizedStage * guStates ]);
		UINT32* puGlobalDecisionAllocationStates = &(gpuGlobalDecisionAllocationStates[ uPrioritizedStage * guStates ]);
		FLOAT* pfProblemHK = &(gpfHKTableValues[ uPrioritizedStage * guProblemStepCount ]);
		FLOAT* pfFlowAllocations = &(gpfFlowAllocations[ uPrioritizedStage * guProblemStepCount ]);

		for( uState = 0; uState < guStates; uState++ )
		{
			// stage can't operate outside of it's feasible region
			UINT32 s = ( uState > gpuLocalMaximumStates[ uPrioritizedStage ] ) ? 0 : uState;
			VERIFY( s < guProblemStepCount );
			if( ex_didFail() ) { goto cleanfailure; }

			puGlobalDecisionAllocationStates[ uState ] = s;

#ifdef ENABLE_MWOBJECTIVE

			pfGlobalDecisionValues[ uState ] = pfProblemHK[ s ] * pfFlowAllocations[ s ]; // refactor & bugfix

#else

			pfGlobalDecisionValues[ uState ] = ( s == 0 ) ? 0 : pfProblemHK[ s ];
		...
		sprintf( gcPrintBuff, "\n%s%c\n", "gpfGlobalDecisionValues", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		sprintf( gcPrintBuff, "%c", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		arr_float_print( stdout, gpfSolutionAllocations, guStates, gcOutputDelimiter );
		sprintf( gcPrintBuff, "%c", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		arr_float_print( stdout, pfGlobalDecisionValues, guStates, gcOutputDelimiter );
		sprintf( gcPrintBuff, "%c", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		arr_uint32_print( stdout, puGlobalDecisionAllocationStates, guStates, gcOutputDelimiter );
#endif
	...
	uPrioritizedStage = gpuStageMap[ guStages - 1 ];

	/* uFeasibleRegionLowerBound is no zero because the backpass starts in stage guStages-2 */
	UINT32 uFeasibleRegionLowerBound = gpuLocalMaximumStates[ uPrioritizedStage ];

	for( uStage = guStages-2; uStage != MAX_UINT; uStage-- ) /* seq */
	{
		uPrioritizedStage = gpuStageMap[ uStage ];
		UINT32 uHigherPriorityStage = gpuStageMap[ uStage + 1 ];
		UINT32 uLocalMaximumState = gpuLocalMaximumStates[ uPrioritizedStage ];
		UINT32 uLocalOptimumState = gpuLocalOptimumStates[ uPrioritizedStage ];
		UINT32 uLocalCoordinationState = uLocalMaximumState * gfCoordinationFactorB;
		UINT32 uFeasibleRegionUpperBound = uFeasibleRegionLowerBound + uLocalMaximumState;

		VERIFY( uFeasibleRegionUpperBound >= uFeasibleRegionLowerBound );
		if( ex_didFail() ) { goto cleanfailure; }

		/* each additional stage will provide its local solutionspace */
		for( uState = 0; uState < guStates; uState++)
		{
			if( uState <= uLocalMaximumState ) {
				...
				gpuLocalDecisions[ uState ] = uState;
			} else {
				...
				UINT32 uOnCamRangeSize = MAX( 1, uLocalMaximumState - uLocalCoordinationState );
				gpuLocalDecisions[ uState ] = uLocalCoordinationState + uState % uOnCamRangeSize;
			}
		}
		...
		for( uScenario = 0; uScenario < MIN( guStates, uFeasibleRegionUpperBound ); uScenario++ ) /* par */
		{
			/* record effects of local decisions */
			for( uState = 0; uState <= uScenario; uState++ ) /* par */
			{
				FLOAT fTol = 1E-9;
				UINT32 uLocalDecision = gpuLocalDecisions[ uState ];

#ifdef ENABLE_MWOBJECTIVE

				/* calculate performance */
				FLOAT fLocalHK = gpfHKTableValues[ uPrioritizedStage * guProblemStepCount + uLocalDecision ];
				FLOAT fLocalQ = gpfFlowAllocations[ uPrioritizedStage * guProblemStepCount + uLocalDecision ];
				FLOAT fLocalMW = fLocalQ * fLocalHK;
				if( fLocalMW < fTol ) { fLocalMW = 0; uLocalDecision = 0; }

				gpuScenarioDecisionStates[ uScenario * guStates + uState ] = uLocalDecision;

				{
					UINT32 uGlobalDecision = uScenario - uLocalDecision;
					FLOAT fGlobalMW = gpfGlobalDecisionValues[ uHigherPriorityStage * guStates + uGlobalDecision ];

					gpfScenarioDecisionValues[ uScenario * guStates + uState ] = fLocalMW + fGlobalMW;
				}

#else

				/* calculate performance */
				FLOAT fLocalDecision = gpfHKTableValues[ uPrioritizedStage * guProblemStepCount + uLocalDecision ];
				if( fLocalDecision < fTol ) { fLocalDecision = 0; uLocalDecision = 0; }

				gpuScenarioDecisionStates[ uScenario * guStates + uState ] = uLocalDecision;

				{
					UINT32 uGlobalDecision = uScenario - uLocalDecision;
					FLOAT fGlobalDecision = gpfGlobalDecisionValues[ uHigherPriorityStage * guStates + uGlobalDecision ];
					FLOAT fDenom = ( fLocalDecision > fTol && fGlobalDecision > fTol ) ? 2.0 : 1.0;

					gpfScenarioDecisionValues[ uScenario * guStates + uState ] = ( fLocalDecision + fGlobalDecision ) / fDenom;
				}
		...
		uFeasibleRegionLowerBound += gpuLocalMaximumStates[ uPrioritizedStage ];

		/* sync */

#ifdef DEBUG_BACKWARDPASS
		sprintf( gcPrintBuff, "\n%s%c", "gpfScenarioDecisionValues", gcOutputDelimiter );
		PRINT_STDOUT( gcPrintBuff );
		arr_float_print( stdout, gpfSolutionAllocations, guStates, gcOutputDelimiter );
		for( uScenario = 0; uScenario < guStates; uScenario++ ) /* par */
		{
			fprintf( stdout, "%f%c ", gpfSolutionAllocations[ uScenario ], gcOutputDelimiter );
#if 1
			fprintf( stdout, "\ngpfScenarioDecisionValues\n" );
			arr_float_print( stdout, &(gpfScenarioDecisionValues[ uScenario * guStates ]), uScenario, gcOutputDelimiter );
			fprintf( stdout, "\ngpuScenarioDecisionStates\n" );
			arr_uint32_print( stdout, &(gpuScenarioDecisionStates[ uScenario * guStates ]), uScenario, gcOutputDelimiter );
			fprintf( stdout, "\n" );
#else
			arr_float_print( stdout, &(gpfScenarioDecisionValues[ uScenario * guStates ]), guStates, gcOutputDelimiter );
#endif
		}
#endif
		...
		for( uScenario = 0; uScenario < guStates; uScenario++ ) /* par */
		{
			UINT32 uStateOfMax = MAX_UINT;
			arr_float_max(
				&(gpfValueOfMax[ uScenario ]),
				&(uStateOfMax),
				&(gpfScenarioDecisionValues[ uScenario * guStates ]),	/* [ states, states ] but actually triangular */
				uScenario + 1,											/* NOTE + 1 to make a count */
				1 );
			gpuStateOfMax[ uScenario ] = gpuScenarioDecisionStates[ uScenario * guStates + uStateOfMax ]; /* [ states, states ] but actually triangular */
		}

#ifdef DEBUG_BACKWARDPASS
		sprintf( gcPrintBuff, "\n%s%c", "gpuStateOfMax", gcOutputDelimiter );
		PRINT_STDOUT( gcPrintBuff );
		arr_float_print( stdout, gpfSolutionAllocations, guStates, gcOutputDelimiter );
		for( uScenario = 0; uScenario < guStates; uScenario++ ) /* par */
		{
			fprintf( stdout, "%f%c ", gpfSolutionAllocations[ uScenario ], gcOutputDelimiter );
			arr_uint32_print( stdout, &(gpuStateOfMax[ uScenario ]), guStates - uScenario, gcOutputDelimiter );
		}
#endif
		/* sync */
		...
		arr_float_copy(
			&(gpfGlobalDecisionValues[ uPrioritizedStage * guStates ]),			/* [ stages, states ] */
			gpfValueOfMax,
			guStates );
 		arr_uint32_copy(
			&(gpuGlobalDecisionAllocationStates[ uPrioritizedStage * guStates ]),	/* [ stages, states ] */
			gpuStateOfMax,
			guStates );
...
#ifdef DEBUG_BACKWARDPASS
		sprintf( gcPrintBuff, "\n%s%c", "gpfGlobalDecisionValues", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		arr_float_print( stdout, gpfSolutionAllocations, guStates, gcOutputDelimiter );
		sprintf( gcPrintBuff, "%c", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		arr_float_print( stdout, &(gpfGlobalDecisionValues[ uPrioritizedStage * guStates ]), guStates, gcOutputDelimiter );
		sprintf( gcPrintBuff, "%c", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		arr_uint32_print( stdout, &(gpuGlobalDecisionAllocationStates[ uPrioritizedStage * guStates ]), guStates, gcOutputDelimiter );
#endif
	}
	...
	for( uStage = 0; uStage < guStages; uStage++ ) /* par */
	{
		arr_float_copy_indirect(
			...
			&(gpuGlobalDecisionAllocationStates[ uStage * guStates ]),	 /* [ stages, states ] */
			guStates );
	}
	...
	{
		sprintf( gcPrintBuff, "%s%c", gpTurbs[ uStage ].szName, gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
		arr_uint32_print( stdout, &(gpuGlobalDecisionAllocationStates[ uStage * guStates ]), guStates, gcOutputDelimiter );
	}

	sprintf( gcPrintBuff, "\n%s%c", "gpfHKTableValues", gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
	arr_float_print( stdout, gpfFlowAllocations, guProblemStepCount, gcOutputDelimiter );
	arr_float_print( stdout, gpfSolutionAllocations, guProblemStepCount, gcOutputDelimiter );
	for( uStage = 0; uStage < guStages; uStage++ )
	{
		sprintf( gcPrintBuff, "%s%c", gpTurbs[ uStage ].szName, gcOutputDelimiter ); PRINT_STDOUT( gcPrintBuff );
	...
	for( uStage = 0; uStage < guStages; uStage++ )
	{
		arr_uint32_set( &(gpuSolution[ uStage * guStates ]), MAX_UINT, guStates );
	}
	...
	for( uStage = 0; uStage < guStages; uStage++ ) /* par */
	{
		gpuStageChecklist[ uStage ] = ( gpTurbs[ uStage ].fWeight < fTol ) ? MAX_UINT : uStage;
	}

	// for each state, select the best way to make the stage allocations
	assert( gpuStageChecklist );
	assert( gpuStageChecklistCopy );
	assert( gpuGlobalDecisionAllocationStates );
	assert( gpuLocalOptimumStates );
	assert( gpuSolution );
	for( uState = 0; uState < guStates; uState++ )
	{
#if 0
		if( uState > guStages * (guProblemStepCount-1) )
		{
			sprintf( gcPrintBuff, "condition at state %lu: maximum domain of problem reached.\n", uState );
			PRINT_STDOUT( gcPrintBuff );
			break;
		}
#endif
		// reinit the checklist
		memcpy( gpuStageChecklistCopy, gpuStageChecklist, guStages * sizeof(UINT32) );
		...
		UINT32 uAllocationRemaining = uState;
		while( uAllocationRemaining != 0 )
		{
			FLOAT fMaxValue = 0;
			UINT32 uStage = MAX_UINT;
			arr_float_max_unmarked_prioritized(
					&fMaxValue,
					&uStage,
					&(gpfGlobalDecisionValues[ 0 * guStates + uAllocationRemaining ]), /* [ stages, states ] */
					gpuStageChecklistCopy, /* [ stages ] */
					gpuStageMap, /* [ stages ] */
					guStages,
					guStates /* stride */ );
			if( uStage == MAX_UINT )
			...
			{
				VERIFY( gpuStageChecklistCopy[ uStage ] != MAX_UINT ); // not in use
				if( ex_didFail() ) { goto cleanfailure; }

				gpuStageChecklistCopy[ uStage ] = MAX_UINT; // don't use this again
				UINT32 uAllocationForThisStage = gpuGlobalDecisionAllocationStates[ uStage * guStates + uAllocationRemaining ];

				VERIFY( uAllocationForThisStage < guProblemStepCount );
#ifdef ENABLE_ADAPTIVEFAILURE
				if( !(uAllocationForThisStage < guProblemStepCount) )
				{
					uAllocationForThisStage = guProblemStepCount - 1;
					ex_clear();
				}
#else
				if( ex_didFail() ) { return; }
				...
				gpuSolution[ uStage * guStates + uState ] = uAllocationForThisStage;
				uAllocationRemaining -= uAllocationForThisStage;

#ifdef DEBUG_FORWARDPASS
				sprintf( gcPrintBuff, "stage %lu: allocated %lu (%f)\n",
					uStage, uAllocationForThisStage, fMaxValue );
				PRINT_STDOUT( gcPrintBuff );
#endif
	...
	// map the solution to the flow and power values
	{
		...
		assert( pPower );
		assert( gpuSolution );
		assert( gpfFlowAllocations );
		assert( gpfPowerAllocations );
		...
		for( uStage = 0; uStage < guStages; uStage++ ) /* par */
		{
			arr_float_copy_indirect(
				...
				&(gpfFlowAllocations[ uStage * guProblemStepCount ]),	/* [ stages, steps ] */
				&(gpuSolution[ uStage * guStates ]),					/* [ stages, states ] */
				guStates );
	...
	for( uStage = 0; uStage < guStages; uStage++ ) /* par */
	{
		arr_float_mark_nan_as_zero( &(gpfHKSolution[ uStage * guStates ]), guStates );
		arr_float_mark_inf_as_zero( &(gpfHKSolution[ uStage * guStates ]), guStates );
	}
#endif

	// summarise flow and power
	arr_float_sum_stages( gpfSolutionAllocations, gpfSolution, guStates, guStages ); /* [ states ] */
	arr_float_sum_stages( gpfOtherSolutionAllocations, gpfOtherSolution, guStates, guStages ); /* [ states ] */

	// summarise HK
	arr_float_average_stages( gpfHKSolutionAverages, gpfHKSolution, guStates, guStages );
	return;

cleanfailure:
	for( uStage = 0; uStage < guStages; uStage++ )
	{
		arr_uint32_set( &(gpuSolution[ uStage * guStates ]), 0, guStates ); /* [ stages, states ] */
...
void dp_print_solution()
{
	...
	if( guTransposeSolution == 0 ) {
		...
		if( guDebugMode )
		{
			...
			sprintf( gcPrintBuff, "\n%s%c", "DecisionValues", gcOutputDelimiter );
			PRINT_STDOUT( gcPrintBuff );
			arr_float_print( stdout, gpfSolutionAllocations, guStates, gcOutputDelimiter );
			for( uStage = 0; uStage < guStages; uStage++ )
			{
			...
			sprintf( gcPrintBuff, "\n%s%c", "DecisionAllocations", gcOutputDelimiter );
			PRINT_STDOUT( gcPrintBuff );
			arr_float_print( stdout, gpfSolutionAllocations, guStates, gcOutputDelimiter );
			for( uStage = 0; uStage < guStages; uStage++ )
			{
			...
			sprintf( gcPrintBuff, "\n%s%c", "DecisionAllocationStates", gcOutputDelimiter );
			PRINT_STDOUT( gcPrintBuff );
			arr_float_print( stdout, gpfSolutionAllocations, guStates, gcOutputDelimiter );
			for( uStage = 0; uStage < guStages; uStage++ )
			{
			...
			sprintf( gcPrintBuff, "\nHKTableBy%s(Weighted)%c", szDecisionVariable, gcOutputDelimiter );
			PRINT_STDOUT( gcPrintBuff );
			arr_float_print( stdout, gpfSolutionAllocations, guProblemStepCount, gcOutputDelimiter );
			sprintf( gcPrintBuff, "\nHKTableBy%s(Weighted)%c", szOtherDecisionVariable, gcOutputDelimiter );
			PRINT_STDOUT( gcPrintBuff );
			arr_float_print( stdout, gpfOtherSolutionAllocations, guStates, gcOutputDelimiter );
			for( uStage = 0; uStage < guStages; uStage++ )
			{
		...
		for( uState = 0; uState < guStates; uState++ )
		{
			...
			if( guDebugMode )
			{
				...
				float_print( stdout, gpfSolutionAllocations[ uState ], gcOutputDelimiter );
				arr_uint32_print_t( stdout, &(gpuGlobalDecisionAllocationStates[ uState ]), guStages, guStates, gcOutputDelimiter );
				...
				if( uState < guProblemStepCount ) {
					...
					putc( gcOutputDelimiter, stdout ); /* skip column */
					for( UINT32 i = 0; i < guStages; i++ ) { putc( gcOutputDelimiter, stdout ); }
				}
			}
...
EXTERNC void ud_dispatch()
{
	...
	/* accomodate problematic table construction... */
	{
		...
		while( uUsableStates != MAX_UINT && gpfSolutionAllocations[ uUsableStates ] < 1.0 ) { uUsableStates--; }
		VERIFY( uUsableStates != MAX_UINT );
		if( ex_didFail() )
		{
		...
		else
		{
			if( strcasecmp( tok, "unit" ) == 0 )
			{
				...
				turbine_register( pUnitName, szCurveName, ft, cfs, kw );
				{
					...
					if( uUnitNumber == MAX_UINT ) { goto parseerror; }
#if 1
					while( 1 )
					{
						FLOAT fValue;
